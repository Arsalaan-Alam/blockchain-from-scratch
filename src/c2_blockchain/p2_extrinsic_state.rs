...
impl Header {
    ...
    fn verify_sub_chain(&self, chain: &[Header]) -> bool {
        let mut top = self; 
        for block in chain {
            if block.parent != hash(top) {
                return false;
            }
            if block.height != top.height + 1 {
                return false;
            }
            if block.state != top.state + block.extrinsic {
                return false;
            }
            top = block;
        }
...
fn build_valid_chain(n: u64) -> Vec<Header> {
    ...
    for _ in [1..n] {
        chain.push(chain.last().unwrap().child(0))
    }
    chain
}
...
fn build_an_invalid_chain() -> Vec<Header> {
    let mut chain = vec![Header::genesis()];
    chain.push(chain.last().unwrap().child(1));
    chain.push(chain.first().unwrap().child(1));
    chain
}
